...
class EnhancedSmartMoneyScreener:
    ...
    def calculate_composite_score(self, row: pd.Series, tech: Dict, fund: Dict, analyst: Dict, rs: Dict) -> Tuple[float, str]:
        ...
        # Determine grade
        if composite >= 80: grade = "S (Immediate Buy)"
        elif composite >= 70: grade = "A (Strong Buy)"
        elif composite >= 60: grade = "B (Buy)"
        elif composite >= 50: grade = "C (Hold)"
        elif composite >= 40: grade = "D (Caution)"
        else: grade = "F (Avoid)"
...
def main():
    ...
    if not results.empty:
        ...
        print("=" * 80)
        print(results[['rank', 'ticker', 'name', 'grade', 'composite_score', 'current_price']].head(args.top).to_string())
